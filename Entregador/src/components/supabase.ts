...
const initSupabase = () => {
  if (!supabaseInstance) {
    ...
    if (!supabaseUrl || typeof supabaseUrl !== 'string' || !supabaseUrl.startsWith('https://')) {
      console.error('URL do Supabase inválida ou não encontrada nas variáveis de ambiente')
    ...
    if (!supabaseAnonKey || typeof supabaseAnonKey !== 'string') {
      console.error('Chave anônima do Supabase não encontrada nas variáveis de ambiente')
