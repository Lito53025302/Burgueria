...
async function runMigration() {
    console.log('🚀 Executando migração de campos de prêmio em orders...');

    const sql = `
        ALTER TABLE public.orders 
        ADD COLUMN IF NOT EXISTS won_reward BOOLEAN DEFAULT FALSE,
    ...
    try {
        ...
        if (error) {
            ...
            console.log('✅ Migração de campos de prêmio concluída com sucesso!');
        }
    } catch (err) {
