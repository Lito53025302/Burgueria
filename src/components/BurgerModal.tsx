...
const BurgerModal = ({ burger, onClose, onAddToCart }: BurgerModalProps) => {
  ...
  const getCustomizationOptions = () => {
    // Usa apenas customizações vindas do banco para evitar complementos "fantasma"
    return burger.customizations && burger.customizations.length > 0 ? burger.customizations : [];
  };
  ...
  return (
    <div
      className="fixed inset-0 z-[9999] bg-black/85 flex items-end md:items-center justify-center overflow-hidden"
      onClick={onClose}
    >
      <div
        className="bg-gray-900 rounded-t-[2rem] md:rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border-t md:border border-gray-800"
      ...
      >
        ...
        <div className="p-6 md:p-8 pb-24 md:pb-8">
          ...
          {/* Customizations */}
          {burger.category !== 'drinks' && customizationOptions.length > 0 && (
            <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 p-6 bg-gray-800/30 rounded-2xl border border-gray-700">
