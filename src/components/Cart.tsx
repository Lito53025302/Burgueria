...
const Cart = ({ isOpen, onClose, items, onUpdateQuantity, onRemoveItem, onClearCart, totalPrice, onOrderComplete, openingTime, closingTime }: CartProps) => {
  ...
  const isStoreOpen = useMemo(() => {
    ...
    if (Number.isNaN(openHours) || Number.isNaN(openMinutes) || Number.isNaN(closeHours) || Number.isNaN(closeMinutes)) {
      return true;
    }
    ...
    return nowMinutes >= openingMinutes || nowMinutes < closingMinutes;
  }, [closingTime, currentTime, openingTime]);
        ...
        {items.length > 0 && (
          <div className="p-6 border-t border-gray-800 bg-gray-900/95 backdrop-blur-sm">
            ...
            <button
              ...
              className={`w-full py-4 font-bold text-lg rounded-xl transition-all duration-300 ${isStoreOpen
                ? 'bg-gradient-to-r from-yellow-500 to-orange-500 text-black hover:scale-[1.02] hover:shadow-lg hover:shadow-yellow-500/25 active:scale-95'
            ...
            {!isStoreOpen && (
              <p className="text-xs text-red-400 text-center mt-3">
                Loja fechada no momento. Faça o pedido no horário de funcionamento.
              </p>
            )}
