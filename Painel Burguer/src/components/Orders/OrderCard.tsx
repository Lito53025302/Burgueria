import { Clock, User, Phone, MapPin, CreditCard, CheckCircle, XCircle, Gift } from 'lucide-react';
import { Order } from '../../types';

interface OrderCardProps {
  order: any; // Usando any temporariamente para evitar conflitos de tipos entre hooks
...
export function OrderCard({ order, onStatusChange, maxPrepTime }: OrderCardProps) {
  ...
  const formatAddress = (address: unknown) => {
    ...
    const { street, number, neighborhood, city, zipCode, complement } = address as AddressData;
    return `${street || ''}, ${number || ''}${complement ? ' - ' + complement : ''} - ${neighborhood || ''}, ${city || ''} ${zipCode ? '- ' + zipCode : ''}`
      .replace(/\s{2,}/g, ' ')
      .trim();
  };
  ...
  return (
    <div className={`bg-white rounded-xl shadow-sm border-2 transition-all border-gray-200 
      ...
      <div className="p-6">
        {/* Prêmio Ganho */}
        {order.won_reward && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-center gap-3 animate-bounce-subtle">
            <div className="bg-yellow-400 p-2 rounded-full">
              <Gift className="h-5 w-5 text-yellow-900" />
            </div>
            <div>
              <p className="text-xs font-bold text-yellow-800 uppercase tracking-wider">🎁 Cliente Ganhou no Jogo!</p>
              <p className="text-sm font-semibold text-yellow-900">{order.reward_description || 'Brinde Especial'}</p>
            </div>
          </div>
        )}
