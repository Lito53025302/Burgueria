...
const Contact = () => {
  ...
  useEffect(() => {
    async function fetchStoreInfo() {
      ...
      if (error) {
        logger.error('Erro ao buscar informações da loja', error);
      } else {
        setStoreInfo(data);
      }
      setLoading(false);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    logger.info('Formulário de contato enviado', { name, email });
  ...
  const hasValidAddress = storeInfo.endereco && storeInfo.endereco.trim().length > 0;
  const mapSrc = hasValidAddress 
    ? `https://www.google.com/maps/embed/v1/place?key=${apiKey}&q=${encodeURIComponent(storeInfo.endereco)}`
    : '';
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="space-y-8 flex flex-col">
            ...
            {/* Mapa - só mostra se houver endereço válido */}
            {hasValidAddress ? (
              ...
              <div className="w-full h-64 md:h-80 rounded-2xl overflow-hidden border border-gray-700 bg-gray-800/50 flex items-center justify-center">
                <div className="text-center text-gray-500">
                  <MapPin className="w-12 h-12 mx-auto mb-2 opacity-50" />
                  <p>Mapa não disponível</p>
                  <p className="text-sm">Configure o endereço da loja</p>
                </div>
              </div>
            )}
          </div>
