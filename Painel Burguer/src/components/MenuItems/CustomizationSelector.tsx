...
type CustomizationInsertResponse = {
    name: string;
    price: number;
};
...
export function CustomizationSelector({ selectedCustomizations, onCustomizationsChange }: CustomizationSelectorProps) {
    ...
    const fetchAvailableCustomizations = async () => {
        try {
            ...
            logger.error('Erro ao carregar complementos', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAddNewCustomization = async () => {
        ...
        try {
            ...
            onCustomizationsChange([
                ...selectedCustomizations,
                {
                    name: (data as CustomizationInsertResponse).name,
                    price: (data as CustomizationInsertResponse).price.toString()
                }
            ]);
            ...
            alert(`❌ Erro: ${error instanceof Error ? error.message : 'Erro ao salvar complemento'}`);
        }
    };
