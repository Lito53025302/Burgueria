...
export function StoreProvider({ children }: { children: ReactNode }) {
  const { tenantId } = useTenantContext(); // ← OBTER TENANT ID
  ...
  const isGalleryImagesColumnMissing = (error: unknown) => {
    if (!error || typeof error !== 'object') return false;
    const message = 'message' in error && typeof error.message === 'string' ? error.message.toLowerCase() : '';
    const details = 'details' in error && typeof error.details === 'string' ? error.details.toLowerCase() : '';
    return message.includes('gallery_images') || details.includes('gallery_images');
  };
  ...
  const addMenuItem = async (item: Omit<MenuItem, 'id' | 'createdAt' | 'soldCount'>) => {
    try {
      ...
      const dbItem = {
        tenant_id: tenantId, // ← FILTRO TENANT
        name: item.name,
      ...
      let { data, error } = await supabase
        .from('menu_items')
        .insert([dbItem])
        .select('id, name, description, price, image, gallery_images, category, available, sold_count, created_at, customizations, spice_level, prep_time_min')
        .single();

      // Compatibilidade: banco sem a migration de gallery_images
      if (error && isGalleryImagesColumnMissing(error)) {
        const dbItemWithoutGallery = {
          tenant_id: tenantId,
          name: item.name,
          description: item.description,
          price: item.price,
      ...
      if (data) {
        const typedData = data as DbMenuItemRow;
        const mappedItem: MenuItem = {
          ...
          galleryImages: typedData.gallery_images || [],
          category: typedData.category,
          available: typedData.available,
          soldCount: typedData.sold_count,
          createdAt: typedData.created_at,
          customizations: typedData.customizations,
          spiceLevel: typedData.spice_level,
          prepTimeMin: typedData.prep_time_min
        };
  ...
  const updateMenuItem = async (id: string, updates: Partial<MenuItem>) => {
    try {
      ...
      // Compatibilidade: banco sem a migration de gallery_images
      if (error && isGalleryImagesColumnMissing(error)) {
        delete dbUpdates.gallery_images;
        const fallbackResult = await supabase
          .from('menu_items')
          .update(dbUpdates)
          .eq('id', id);
  ...
  useEffect(() => {
    async function fetchMenuItems() {
      if (!tenantId) return; // ← Aguardar tenant ser carregado
      ...
      if (data) {
        const mappedItems = data.map((item) => {
          const typedItem = item as DbMenuItemRow;
          return {
            id: typedItem.id,
            name: typedItem.name,
            ...
            galleryImages: typedItem.gallery_images || [],
            category: typedItem.category,
            available: typedItem.available,
            soldCount: typedItem.sold_count,
            createdAt: typedItem.created_at,
            customizations: typedItem.customizations,
            spiceLevel: typedItem.spice_level,
            prepTimeMin: typedItem.prep_time_min
          };
        });
    ...
    async function fetchOrders() {
      ...
      const mappedOrders = (data || []).map((order) => {
        const typedOrder = order as DbOrderRow;
        return {
          id: typedOrder.id,
          customerName: typedOrder.customer_name,
          customerPhone: typedOrder.customer_phone,
          items: typeof typedOrder.items === 'string' ? JSON.parse(typedOrder.items) : typedOrder.items,
          total: typedOrder.total,
          status: typedOrder.status,
          createdAt: typedOrder.created_at,
          estimatedTime: typedOrder.estimated_time,
          address: typedOrder.address,
          paymentMethod: typedOrder.payment_method,
          changeFor: typedOrder.change_for,
          motoboy_id: typedOrder.motoboy_id,
          motoboyName: typedOrder.motoboy_name, // se existir na tabela, senão ficará undefined
          motoboy_arrived: typedOrder.motoboy_arrived,
        };
      });
